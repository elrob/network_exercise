...
void Router::send( const Message &m ) { 
  ...
  if ( it != rTable.end( ) )
	...
	{
	  forward_address = shortest_route_fwd_address( m.getDestination() );
	  cout << "Router " << address << " forwarded a message to Router " << forward_address << ".\n" << endl;
...
/* BEGIN FUNCTION DEFINITION */
int Router::shortest_route_fwd_address( int dest ) {
  pair< multimap< int, list< int > >::iterator, multimap< int, list< int > >::iterator> ret;
  ret = rTable.equal_range( dest );
  multimap< int, list<int> >::iterator it, shortest_route_it;
  shortest_route_it = ret.first;

  for ( it = ret.first; it != ret.second; it++ )
    if ( (*it).second.size() < (*shortest_route_it).second.size() )
      shortest_route_it = it;

  int fwd_address =  *((*shortest_route_it).second.begin());
